...
if __name__ == "__main__":
    ...
    # eval_env_type - 'AlfredTWEnv'. 
    for eval_env_type in eval_envs:
        # controller_type is either "oracle" or "mrcnn_astar"
        for controller_type in (controllers if eval_env_type == "AlfredThorEnv" else ["tw"]):
            # Eval_paths are here : '$ALFWORLD_DATA/json_2.1.1/valid_seen'
            for eval_path in eval_paths:
                ...
                # Get environment.eval_env_type, which is usually environment.AlfredThorEnv: config and train_eval are the input parameters or something.
                alfred_env = getattr(environment, config["general"]["evaluate"]["env"]["type"])(config,
                                                                                                train_eval="eval_out_of_distribution")
                # Init the new environment with batch_size 1
                ...
                for i in range(num_games):
                    ...
                    obs, info = env.reset()
                    # agent_class has the option of either being `BaselineAutogenAgent` or `GWTAutogenAgent`
                    agent = agent_class(env, obs, info, llm_config, log_path=base_path, game_no=i, max_actions=30,
                                        args=args)

                    # contains a bunch of paths to local history documentation documents, such as `task`, `history`, `guidance`, `admissible_commands_path`
                    # `chat_history_path`, `message_path`, `results_path`, etc etc
                    log_paths = agent.get_log_paths()

                    # Building the initial message (initial prompt) that we feed to the LLM model. 
                    initial_message_content = ""
                    # find the task description in the observation, save it as a txt file.
                    task_description = obs[0].split("Your task is to: ")[1]

                    # Observation comes before the phrase "Your task is to: "
                    initial_observation = obs[0].split("Your task is to: ")[0].split("\n\n")[1]

                    # write task description into file represented by log_paths['task_path']
                    ...
                    # agent, which is either autogen_agent or baseline_agent
                    ...
                    # save the admissible commands (i.e. commands in the ALFWorld Env) into a txt file
                    admissible_commands = list(info['admissible_commands'][0])

                    # Get the path of the admissible commands file and save everything into there
                    with open(log_paths['admissible_commands_path'], "w") as f:
                        f.write(f"{admissible_commands}\n")

                    # Add the admissible commands (the commands that are output from the info)
                    # to the message that will be sent into the LLM
                    ...
                    print("Run chat")
                    try:
                        ## Use the initial message content as the initial message
                        ## start the group chat, obtain the chat_result and the error_message (if any)
                        ## Chat result should contain list of chat histories, or be None if there is no LLM
                    ...
                    if error_message is not None:
                        ## Load error message into the error_message_path file
